...
def chk_s008(filename,name):
    for line_n, line in enumerate(filename,1):  
        not_comment, _ = split_comment(line)
        if not_comment:
            if re.match(r"[ ]*class( ){1,}[a-z_].*[(:]",not_comment) or re.match(r"[ ]*class( ){1,}[A-Z].*_.*[(:]",not_comment):
...
def chk_s010_011_012(filename,name):
    tree = ast.parse("\n".join(filename))
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            if re.match(r"_*[A-Z].*",node.name):
                ErrorList(node.lineno,f"S009 Function name '{node.name}' should use snake_case",name)
            arguments = [a.arg for a in node.args.args]
            for arg in arguments:
                if re.match(r"_*[A-Z].*",arg):
                    ErrorList(node.lineno,f"S010 Argument name '{arg}' should be snake_case",name)
            mutable_def_args = [isinstance(a,ast.List) for a in node.args.defaults]
            for index,mutable_arg in enumerate(mutable_def_args[::-1]):
                if mutable_arg:
                    ErrorList(node.lineno,f"S012 Default argument '{arguments[-index]}' is mutable",name)
                    break
            var_names = []
            var_line = []
            for assgn_node in ast.walk(node):
                if isinstance(assgn_node, ast.Assign):
                    var_names = var_names + [b.id for b in assgn_node.targets]
                    var_line = var_line + [assgn_node.lineno]
            for index,var_name in enumerate(var_names):
                if re.match(r"_*[A-Z].*",var_name):
                    ErrorList(var_line[index],f"S011 Variable name '{var_name}' should be snake_case",name)
